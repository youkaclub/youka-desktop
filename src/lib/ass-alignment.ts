...
export function alignmentsToAss(
...
function fixAlignments(
  alignments: Alignment[],
  { delta, fixDelta }: { delta: number; fixDelta: number }
): Alignment[] {
...
function alignmentsByLine(
  alignments: Alignment[]
): Record<number, Alignment[]> {
  let lineIndex = 1;
...
function interleaveLines(
  ...
  lines.forEach((line, i) => {
    ...
    const previousGap = previousLine
      ? line.start - previousLine.end
      : line.start;
    const nextGap = nextLine ? nextLine.start - line.end : 0;
    const position =
      previousGap > breakLength || previousLine?.position === "top"
        ? "bottom"
        : "top";
